...
NB_smpKit {
	...
	classvar <kitGroup, <kitVoices, <kitBuffers;
	...
	*addPlayer {
		...
		if (kitGroup.isNil) {
			...
			SynthDef(\smpkit_mono,{
				...
				sDur = (numFrames * lenRel / rate.abs / sR) - (fadeIn + fadeOut);
	...
	*queueLoadSample { arg vox, path;
		...
		if (loadingSamples.not) { NB_smpKit.loadSample() };
	}
	...
	*loadSample {
		...
		if (loadQueue.notEmpty) {
			...
			NB_smpKit.clearSample(t.vox);
			kitBuffers[t.vox] = Buffer.read(Server.default, t.path, action: { NB_smpKit.loadSample() });
		}{
	...
	*initClass {
		...
		var voiceParams, numVoices = 12;

		StartUp.add {

			voiceParams = Array.fill(numVoices, {
				Dictionary.newFrom([
					\mainAmp, 1,
					\amp, 0.8,
					\pan, 0,
			...
			kitVoices = Array.newClear(numVoices);
			kitBuffers = Array.newClear(numVoices);
			...
			OSCFunc.new({ |msg|
				if (kitGroup.notNil) { kitGroup.set(\gate, -1.05) }
			}, "/nb_smpkit/panic");
			...
			OSCFunc.new({ |msg|
				loadQueue = Array.new(numVoices);
				loadingSamples = false;
			}, "/nb_smpkit/reset_loadqueue");

			OSCFunc.new({ |msg|
				var vox = msg[1].asInteger;
				var path = msg[2].asString;
				NB_smpKit.queueLoadSample(vox, path)
