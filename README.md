...
- three play modes (`oneshot`, `hold`, `toggle`)
...
- smpKit initializes in an empty state. Complete states can be recalled under `presets`. When smpKit is active and a pset of the active script is saved, all parameters are saved as well. Total recall can be achieved either with psets or via save/load.
...
- Under `modulation`, global modulation for `drive`, `lpf cutoff`, `hpf cutoff`, `send a`, and `send b` is accessed. Set the relative change ±0–100% of the parameters that will be affected either via `nb:modulate` (e.g., with sidvagn) or by MIDI-mapping the `mod amt` parameter.
